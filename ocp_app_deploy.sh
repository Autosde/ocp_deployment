...
helm upgrade ${RELEASE_NAME} ${CHART_PATH} ${HELM_TLS_OPTION} --install --debug --dry-run --set env.DB_USER=${DB_USER},env.DB_PASSWORD=${DB_PASSWORD},env.IAM_ENDPOINT=${IAM_ENDPOINT},env.SERVICE_OPERATOR_API_KEY=${SERVICE_OPERATOR_API_KEY},env.RESOURCE_CONTROLLER_URL=${RESOURCE_CONTROLLER_URL},env.DB_NAME=${DB_NAME},env.DB_HOST=${DB_HOST},env.DB_PORT=${DB_PORT},image.repository=${IMAGE_REPOSITORY},image.tag=${IMAGE_TAG},image.pullSecret=${IMAGE_PULL_SECRET_NAME} ${HELM_UPGRADE_EXTRA_ARGS} --namespace ${CLUSTER_NAMESPACE}
...
helm upgrade ${RELEASE_NAME} ${CHART_PATH} ${HELM_TLS_OPTION} --install --set env.DB_USER=${DB_USER},env.DB_PASSWORD=${DB_PASSWORD},env.IAM_ENDPOINT=${IAM_ENDPOINT},env.SERVICE_OPERATOR_API_KEY=${SERVICE_OPERATOR_API_KEY},env.RESOURCE_CONTROLLER_URL=${RESOURCE_CONTROLLER_URL},env.DB_NAME=${DB_NAME},env.DB_HOST=${DB_HOST},env.DB_PORT=${DB_PORT},image.repository=${IMAGE_REPOSITORY},image.tag=${IMAGE_TAG},image.pullSecret=${IMAGE_PULL_SECRET_NAME} ${HELM_UPGRADE_EXTRA_ARGS} --namespace ${CLUSTER_NAMESPACE}
