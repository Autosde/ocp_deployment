...
kubectl delete --ignore-not-found=true secret ${CHART_NAME}-env -n "${CLUSTER_NAMESPACE}"
echo "$APP_ENV" > /tmp/app_env
kubectl create secret generic ${CHART_NAME}-env -n "${CLUSTER_NAMESPACE}" --from-file=/tmp/app_env
...
EOF
  ...
  echo "DEPLOYMENT SUCCEEDED"
    ...
    export APP_URL=https://"${CHART_NAME}.${NLB}" # using 'export', the env var gets passed to next job in stage
